...
In both cases, \btext{more than one model} %several models
...
activity \textsf{Initial Rejection}. \btext{We report here one of the returned models:} %Many models were found, among them the following one:
...
In model $M_1$, the \textsf{precedence} constraint implies that if a biopsy is executed, then the colposcopy examination has provided an uncertain result before. The second model is identical to the first one, except for the constraint \btext{related} %subjected
to our preference. Interestingly, the \textsf{succession} \btext{relates} %is about
the same activities involved \btext{in} %by
the \textsf{precedence} constraint in the first model. The difference between the two models lies in the logical relation between \textsf{precedence} and \textsf{succession}: a trace that violates the former will always violate the latter (but not \btext{vice versa}% the opposite
