...
\end{align*}
%
where the positive example set $P$ contains only one trace (\btext{composed of}
%lasting
 three activities), and the negative example set $N$ contains a single trace as well.

\noindent \btext{If the employee is an } 
%An
 employee working in the marketing department\btext{, she could have } 
%has
...
\end{align*}
%
Alternative models allowing us to \btext{represent the traces that belong to the set $P$ and exclude the ones that belong to the set $N$ are:}%distinguish traces belonging to the two sets are:
\begin{align*}
...
From \btext{a}%the
 logical viewpoint, models $\mathsf{M_1}$--$\mathsf{M_5}$ are equivalent. However, models $\mathsf{M_2}$--$\mathsf{M_5}$ suggest that \btext{what distinguishes the traces in $P$ from the traces in $N$ } 
%there
 is a relation between \btext{the} activities \textsf{a} and \textsf{b}: \btext{these models contain a binary constraint, whose purpose is indeed to highlight a relation between these two activities. }%indeed, the models contain a binary constraint, whose purpose is indeed to highlight relations between activities.
 Model $\mathsf{M_1}$, instead, does not tell us anything about possible links between activities \textsf{a} and \textsf{b}, and a user might conclude that no relation exists between \btext{them.}%the two activities.
\qed
\end{example}
...
\btext{Let us consider then the case where alternative non-equivalent models are discovered.}
...
\end{align*}
%
Alternative models \btext{that accept the traces in $P$ and discard the ones in $N$ are:}
...
Example \ref{ex:unknownShapedDifferently} shows how traces not appearing in the log used for the discovery might be %then
...
The third type of preference \btext{on} %over
 process models is a straightforward combination of the preference types introduced in Subsections \ref{subsec:prefOverActivities} and \ref{subsec:prefOverTemplates}. Domain-related knowledge would drive the attention to certain activities, and preferences \btext{on template types would allow focusing on certain relation types.}%between the template types would allow to focus on certain relation types.


\begin{example}
\label{ex:prefOverBoth}
Let us consider again the ``loan scenario'' \btext{and the log described above:}%previously introduced, and the same log as well:
%
\begin{align*}
P & = \{\ (\mathsf{loanRequest}, \mathsf{requestEval}, \mathsf{notifyOutcome})\ \} \\
N & = \{\ (\mathsf{requestEval}, \mathsf{loanRequest})\ \}
...
now the viewpoint of an employee working in the internal auditing department. Given that the wrong execution order of certain activities might be a symptom of some fraud, the employee would like to focus the attention \btext{on} %to
templates of type \textsf{response} and/or \textsf{precedence}, and in particular \btext{on} %to
