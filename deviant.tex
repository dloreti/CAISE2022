...
\end{equation}
according to a certain domain-independent \todo{ST: why domain-independent?}criterion. Due to the fact that not all the pairs of negative and positive set of traces can be perfectly separated by Declare~\cite{DBLP:conf/bpm/SlaatsDB21}, there can be traces in $L^-$ for which the ${\textit{\sheriff}}$ is empty; we know that those cannot be excluded by any model that guarantees the acceptance of all the positive ones.
