...
From a theoretical point of view, the \nd approach can be seen as a two-step procedure. % which takes as input:
%\begin{itemize}
...
\end{equation}
%
The \textit{\sheriff} function is used to identify the set of all candidate constraints from which a model can be selected; that is, the set $\mathcal{C}$ of all the constraints in $D[A]$ accepting all positive traces and rejecting at least one negative trace, i.e., $\mathcal{C} = \bigcup_{t\in L^-} \textit{\sheriff}(t)$. The solutions are selected from the set 
\begin{equation}
...
In~\cite{deviant-tkde}, two different criteria were taken into account: \emph{generality} (or conversely, \emph{specificity}), and \emph{simplicity}.
